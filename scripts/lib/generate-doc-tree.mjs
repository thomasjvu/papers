...
const EXCLUDED_PUBLIC_DOC_PATHS = new Set([
  'install',
  'integrations/browser-based-providers',
  'maintenance/framework-audit',
...
const SECTION_ORDER = {
  ...
  plugins: ['overview', 'developing', 'marketplace', 'troubleshooting'],
};
...
function getDocPath(relativePath) {
  return toPosix(relativePath).replace(/\.(md|mdx)$/i, '');
}

function isExcludedFile(relativePath) {
  return EXCLUDED_PUBLIC_DOC_PATHS.has(getDocPath(relativePath));
}
...
async function buildTree(relativeDir = '') {
  ...
  const files = entries
    ...
    .filter((entry) => !isExcludedFile(relativeDir ? path.join(relativeDir, entry.name) : entry.name))
    .map((entry) => entry.name);
  const directories = entries
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .filter((directoryName) => {
      const childRelativeDir = relativeDir ? path.join(relativeDir, directoryName) : directoryName;
      return !isDirectoryExcluded(childRelativeDir);
    });
